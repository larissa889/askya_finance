...
<div class="glass-card" style="max-width: 800px;">
    ...
    <div class="d-flex align-items-center flex-column flex-sm-row text-center text-sm-start gap-4 pb-4 mb-4" style="border-bottom: 1px solid var(--border-glass);">
        <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=0D8ABC&color=fff&size=128" alt="Avatar" class="rounded-circle" style="width: 90px; height: 90px; border: 3px solid rgba(13, 138, 188, 0.3);">
        <div>
            <h3 class="fw-bold text-white mb-1">{{ $user->name }}</h3>
            <p class="text-primary fw-semibold mb-1" style="font-size: 0.95rem;">Comptable</p>
            <p class="text-muted small mb-0">Compte créé le {{ $user->created_at ? $user->created_at->format('d/m/Y') : '-' }}</p>
        </div>
    </div>

    <!-- Edit Form -->
    <form action="{{ route('comptable.profile.update') }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="d-flex gap-3 mt-5">
            ...
            <a href="{{ route('comptable.dashboard') }}" class="btn-custom">
                Annuler
            </a>
        </div>
    </form>
